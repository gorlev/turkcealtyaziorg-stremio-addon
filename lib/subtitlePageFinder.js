...
module.exports = async function subtitlePageFinder(imdbId,type, season, episode, httpsAgent) {
    
    try {
        let subtitlesData = [];

        //GOES TO THE MAIN PAGE FOR THE MOVIE/SERIES.
        const mainPageURL  = await mainPageFinder(imdbId, httpsAgent)
        if(mainPageURL.length > 0){

            const mainPageHTML = await axios({url:mainPageURL,method:"GET", httpsAgent})
            ...
            for (let i= 0; i <subtitlesData.length; i++) {
                let subIDs = await subIDfinder(subtitlesData[i].pageUrl, httpsAgent)
